...
5. **Severity earned, not assumed.** Blocker = harm on merge, and blocks whatever it changes. Important = degrades quality enough to fix before merge, and blocks only when addressing it changes behavior or a check's ability to fail. Suggestion = optional. Nit = cosmetic. Blocker and important both trigger `changes-requested`; suggestion and nit are comment-only.
